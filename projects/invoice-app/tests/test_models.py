...
class ModelsTests(unittest.TestCase):
    ...
    def test_model_integrity(self):
        ...
        address = Address(street = "100 Main", postal_code = "12345", state = "KA", city = "Bangalore", country="IN")
        session.add(address)
        session.flush()
        print(address)

        user = User(first_name = "Ed", last_name = "Sherin", email = "ed@sample.com", address_id= address.id)
        session.add(user)
        session.flush()
        print(user)
        self.assertTrue(user.id == 1)

        product = Product(name = "Galaxy Phone", family = "Mobile", list_price = 1000.0, inventory = 100, tax_rate = 18.0)
        session.add(product)
        session.flush()
        print(product)
        self.assertTrue(product.id == 1)

        account = Account(name="Samsung", email = "info@samsung.com", tax_id = "123abc", address_id = address.id)
        session.add(account)
        session.flush()
        print(account)
        ...
        invoice_line_item = InvoiceLineItem(invoice_id = 1, product_id = 1, list_price = 1200.0, quantity = 1)
